...
!
!    Input, double precision X(N), the point at which the jacobian is to be
!    evaluated.
!
!    Input, double precision FVEC(M), is used only when MODE = 2.
!    In that case, it should contain the function values at X.
!
!    Input, double precision FJAC(LDFJAC,N), an M by N array.  When MODE = 2,
...
!
!    Output, double precision XP(N), on output with MODE = 1, is a neighboring
!    point of X, at which the function is to be evaluated.
!
!    Input, double precision FVECP(M), on input with MODE = 2, is the function
!    value at XP.
!
!    Input, integer MODE, should be set to 1 on the first call, and
!    2 on the second.
!
!    Output, double precision ERR(M).  On output when MODE = 2, ERR contains
...
!
!    Input, double precision R(LR), the upper triangular matrix R stored
!    by rows.
!
!    Input, integer LR, the size of the R array, which must be 
!    no less than (N*(N+1))/2.
!
!    Input, double precision DIAG(N), the diagonal elements of the matrix D.
!
!    Input, double precision QTB(N), the first N elements of the vector Q'* B.
!
!    Input, double precision DELTA, is a positive upper bound on the
!    euclidean norm of D*X(1:N).
!
!    Output, double precision X(N), the desired convex combination of the
...
!
  ...
  double precision qtb(n)
  double precision r(lr)
  double precision sgnorm
  double precision sum2
  double precision temp
  double precision wa1(n)
  double precision wa2(n)
  double precision x(n)
...
!
!    Input, double precision X(N), the vector whose norm is desired.
!
!    Output, double precision ENORM, the Euclidean norm of the vector.
!
  implicit none
...
!    Input, double precision X(N), the vector whose norm is desired.
!
!    Output, double precision ENORM2, the Euclidean norm of the vector.
!
  implicit none
  ...
  double precision enorm2
  integer i
  double precision rdwarf
  double precision rgiant
  double precision s1
  double precision s2
  double precision s3
  double precision x(n)
  double precision xabs
  double precision x1max
  double precision x3max
...
!
!    Input, double precision X(N), the point where the jacobian is evaluated.
!
!    Input, double precision FVEC(N), the functions evaluated at X.
!
!    Output, double precision FJAC(LDFJAC,N), the N by N approximate
!    jacobian matrix.
!
...
!
!    Input, double precision EPSFCN, is used in determining a suitable step
...
!    Input, double precision X(N), the point where the jacobian is evaluated.
!
!    Input, double precision FVEC(M), the functions evaluated at X.
!
!    Output, double precision FJAC(LDFJAC,N), the M by N approximate
!    jacobian matrix.
!
!    Input, integer LDFJAC, the leading dimension of FJAC, 
...
!    immediately to the calling program, with the value of IFLAG.
!
!    Input, double precision EPSFCN, is used in determining a suitable
...
!    Input, integer N, the number of functions and variables.
!
!    Input/output, double precision X(N).  On input, X must contain an initial
!    estimate of the solution vector.  On output X contains the final
!    estimate of the solution vector.
!
!    Output, double precision FVEC(N), the functions evaluated at the output X.
!
!    Input, double precision XTOL.  Termination occurs when the relative error
...
!    precision.
!
!    Input/output, double precision DIAG(N).  If MODE = 1, then DIAG is set
...
!
!    Input, double precision FACTOR, determines the initial step bound.  This
...
!
!    Output, double precision FJAC(LDFJAC,N), an N by N array which contains
...
!
!    Output, double precision R(LR), the upper triangular matrix produced by
...
!    less than (N*(N+1))/2.
!
!    Output, double precision QTF(N), contains the vector Q'*FVEC.
!
  implicit none
  ...
  double precision wa1(n)
  double precision wa2(n)
  double precision wa3(n)
  double precision wa4(n)
  double precision x(n)
...
!    Output, double precision FVEC(N), the functions evaluated at the output X.
!
!    Input, double precision TOL.  Termination occurs when the algorithm
...
!    Output, double precision FVEC(N), the functions evaluated at the output X.
!
!    Output, double precision FJAC(LDFJAC,N), an N by N matrix, containing
...
!    with IFLAG = 2.
!
!    Output, double precision R(LR), the upper triangular matrix produced
...
!    estimate of the solution vector.
!
!    Output, double precision FVEC(M), the functions evaluated at the output X.
!
!    Output, double precision FJAC(LDFJAC,N), an M by N array.  The upper
...
!    LDFJAC must be at least M.
!
!    Input, double precision FTOL.  Termination occurs when both the actual
...
!    nonnegative.
!
!    Input, double precision GTOL.  Termination occurs when the cosine of the
...
!
!    Output, double precision QTF(N), contains the first N elements of Q'*FVEC.
!
  implicit none
...
!    2, scaling is specified by the input DIAG vector.
!
!    Input, double precision FACTOR, determines the initial step bound.  
...
!
!    Output, double precision QTF(N), the first N elements of Q'*FVEC.
!
  implicit none
...
!
!    Input/output, double precision R(LDR,N),the N by N matrix.  The full
...
!    Input, double precision QTB(N), the first N elements of the vector Q'*B.
!
!    Input, double precision DELTA, an upper bound on the euclidean norm
!    of D*X.  DELTA should be positive.
!
!    Input/output, double precision PAR.  On input an initial estimate of the
!    Levenberg-Marquardt parameter.  On output the final estimate.
!    PAR should be nonnegative.
!
!    Output, double precision X(N), the least squares solution of the system
!    A*X = B, sqrt(PAR)*D*X = 0, for the output value of PAR.
!
!    Output, double precision SDIAG(N), the diagonal elements of the upper
!    triangular matrix S.
!
  implicit none
  ...
  integer nsing
  double precision par
  double precision parc
  double precision parl
  double precision paru
  double precision qnorm
  double precision qtb(n)
  double precision r(ldr,n)
  double precision sdiag(n)
  double precision sum2
  double precision temp
  double precision wa1(n)
  double precision wa2(n)
...
!    Output, double precision FVEC(M), the functions evaluated at the output X.
!
!    Output, double precision FJAC(LDFJAC,N), an N by N array.  The upper
...
!    LDFJAC must be at least N.
!
!    Input, double precision FTOL.  Termination occurs when both the actual and
...
!
!    Input/output, double precision Q(LDQ,M).  Q is an M by M array.
...
!
!    Input/output, double precision A(LDA,N), the M by N array.
...
!
!    Output, double precision RDIAG(N), contains the diagonal elements of R.
!
!    Output, double precision ACNORM(N), the norms of the corresponding
...
!
  ...
  double precision acnorm(n)
  double precision ajnorm
  double precision enorm
  double precision epsmch
  integer i
  integer i4_temp
  ...
  integer minmn
  logical pivot
  double precision r8_temp(m)
  double precision rdiag(n)
  double precision temp
...
!    Input, integer N, the order of R.
!
!    Input/output, double precision R(LDR,N), the N by N matrix.
...
!    Input, double precision QTB(N), the first N elements of the vector Q'*B.
!
!    Output, double precision X(N), the least squares solution.
!
!    Output, double precision SDIAG(N), the diagonal elements of the upper
!    triangular matrix S.
!
...
!    be less than M.
!
!    Input, double precision V(N), W(N), contain the information necessary
...
!    N must not exceed M.
!
!    Input/output, double precision S(LS).  On input, the lower trapezoidal
...
!    least (N*(2*M-N+1))/2.
!
!    Input, double precision U(M), the U vector.
!
!    Input/output, double precision V(N).  On input, V must contain the 
!    vector V.  On output V contains the information necessary to recover the
!    Givens rotations GV described above.
!
!    Output, double precision W(M), contains information necessary to
...
!
  ...
  double precision cotan
  double precision giant
  integer i
  integer j
  integer jj
  integer l
  double precision s(ls)
...
!    Input, integer N, the order of R.
!
!    Input/output, double precision R(LDR,N).  On input the upper triangular
...
!
!    Input, double precision W(N), the row vector to be added to R.
!
!    Input/output, double precision B(N).  On input, the first N elements
!    of the vector C.  On output the first N elements of the vector Q'*C.
!
!    Input/output, double precision ALPHA.  On input, the (N+1)-st element
!    of the vector C.  On output the (N+1)-st element of the vector Q'*C.
!
!    Output, double precision C(N), S(N), the cosines and sines of the
